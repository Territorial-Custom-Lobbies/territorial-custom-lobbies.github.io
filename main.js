...
var inputValues = { ...defaultInputValues};

var players = [];

function handleInputContainer(hide) {
    const inputContainer = document.querySelector(".input-container");
    if (inputContainer && hide) {
...
function createInputBars() {
    let inputContainer = document.createElement("div");
    inputContainer.className = "input-container";

    const inputs = [
        { label: "Map", placeholder: "Map ID", id: "mapID", defaultValue: inputValues.mapID },
        { label: "Game Mode", placeholder: "Game Mode", id: "gameMode", defaultValue: inputValues.gameMode },
        { label: "Difficulty", placeholder: "Difficulty", id: "difficulty", defaultValue: inputValues.difficulty },
        { label: "Code", placeholder: "Code", id: "code", defaultValue: inputValues.code }
    ];
    ...
    inputs.forEach(inputData => {
        ...
        input.value = inputData.defaultValue;
        input.style.flex = "1";
        ...
        input.style.color = "rgba(0,0,0, 0.7)"; // Example color
        input.style.backgroundColor = "inherit"; // Example background color
        input.style.fontSize = "1em";
        input.style.marginBottom = "0px";
        input.style.fontFamily = "Trebuchet MS, sans-serif"; 

        input.addEventListener("input", function(event) {
            inputValues[inputData.id] = event.target.value;
        });
...
function bn() {
  ...
  this.dH = function() {
    ...
    if (ay.jh) {
      ...
      let dif = parseInt(inputValues.difficulty, 10);
      // Ensure dif is a number between 0 and 5
      if (isNaN(dif) || dif < 0 || dif > 5) {
          dif = defaultInputValues.difficulty;
      }
...
function DataDecoder() {
  ...
  function aJe() {
    ...
    for (var z = 0; z < var_playerCount; z++) {
      ...
      var code = nameArray[nameArray.length - 1];
      if (code == inputValues.code) {
        nameArray.pop();
        var modifiedName = nameArray.join(' ');
        if (z == var_myID) {
...
function aIq() {
  this.a7a = function(wR) {
    ...
    var code = inputValues.code;
    var maxUsernameLength = 20 - code.length - 1;
    if (username.length > maxUsernameLength) {
      username = username.slice(0, maxUsernameLength);
    }
    var combinedName = username + ' ' + code;
